  ...
  static inline std::pair<NTSTATUS, HANDLE> ntcreatefile(std::shared_ptr<async_io_handle> dirh, BOOST_AFIO_V1_NAMESPACE::path path, file_flags flags, bool overlapped = true) BOOST_NOEXCEPT
  {
    ...
    if(flags != file_flags::None)
    {
      if(!!(flags & file_flags::int_opening_dir))
      {
        ...
        if(!!(flags & file_flags::OSDirect))
          flags = flags & ~file_flags::OSDirect;
      }
      else
      {
        ntflags |= 0x040 /*FILE_NON_DIRECTORY_FILE*/;
        if(!!(flags & file_flags::Append))
          access |= FILE_APPEND_DATA | DELETE;
        else
        {
          if(!!(flags & file_flags::Read))
            access |= GENERIC_READ;
    ...
    return std::make_pair(NtCreateFile(&h, access, &oa, &isb, &AllocationSize, attribs, fileshare, creatdisp, ntflags, NULL, 0), h);
  }
...
BOOST_AFIO_HEADERS_ONLY_FUNC_SPEC filesystem::path normalise_path(path p, path_normalise type)
{
  ...
  if(needToOpen)
  {
    ...
    BOOST_AFIO_ERRHNTFN(ntstat, [&p]
                        {
                          return p;
                        });
  }
  auto unh = detail::Undoer([&h]
                            {
                              if(h)
                                CloseHandle(h);
                            });
  ...
  else
  {
    ...
    std::sort(dosmountpoints.begin(), dosmountpoints.end(), [](const path::string_type &a, const path::string_type &b)
              {
...
namespace detail
{
  struct async_io_handle_windows : public async_io_handle
  {
    ...
    async_io_handle_windows(async_file_io_dispatcher_base *_parent, const BOOST_AFIO_V1_NAMESPACE::path &path, file_flags flags)
    ...
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC void close() override final
    {
      ...
      if(mapaddr)
      {
        BOOST_AFIO_ERRHWINFN(UnmapViewOfFile(mapaddr), [this]
                             {
                               return path();
                             });
        mapaddr = nullptr;
      ...
      if(h)
      {
        ...
        if(SyncOnClose && write_count_since_fsync())
          BOOST_AFIO_ERRHWINFN(FlushFileBuffers(myid), [this]
                               {
                                 return path();
                               });
        h->close();
...
# endif  /*_ 0 _*/
    ...
    ~async_io_handle_windows() { async_io_handle_windows::close(); }
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC directory_entry direntry(metadata_flags wanted) override final
    {
      ...
      if(((int) needInternal + (int) needBasic + (int) needStandard) >= 2)
      {
        ...
        if(STATUS_PENDING == ntstat)
          ntstat = NtWaitForSingleObject(h, FALSE, NULL);
        BOOST_AFIO_ERRHNTFN(ntstat, [this]
                            {
                              return path();
                            });
      }
    ...
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC BOOST_AFIO_V1_NAMESPACE::path target() override final
    {
      ...
      case IO_REPARSE_TAG_MOUNT_POINT:
        return BOOST_AFIO_V1_NAMESPACE::path(path::string_type(rpd->MountPointReparseBuffer.PathBuffer + rpd->MountPointReparseBuffer.SubstituteNameOffset / sizeof(BOOST_AFIO_V1_NAMESPACE::path::value_type), rpd->MountPointReparseBuffer.SubstituteNameLength / sizeof(BOOST_AFIO_V1_NAMESPACE::path::value_type)),
                                             BOOST_AFIO_V1_NAMESPACE::path::direct());
      case IO_REPARSE_TAG_SYMLINK:
        return BOOST_AFIO_V1_NAMESPACE::path(path::string_type(rpd->SymbolicLinkReparseBuffer.PathBuffer + rpd->SymbolicLinkReparseBuffer.SubstituteNameOffset / sizeof(BOOST_AFIO_V1_NAMESPACE::path::value_type), rpd->SymbolicLinkReparseBuffer.SubstituteNameLength / sizeof(BOOST_AFIO_V1_NAMESPACE::path::value_type)),
                                             BOOST_AFIO_V1_NAMESPACE::path::direct());
      }
    ...
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC void link(const async_path_op_req &_req) override final
    {
      ...
      BOOST_AFIO_ERRHNTFN(NtSetInformationFile(myid, &isb, fni, sizeof(FILE_LINK_INFORMATION) + fni->FileNameLength, FileLinkInformation), [this]
                          {
                            return path();
                          });
    }
...
# else  /*_ 0 _*/
      ...
      do
      {
        ...
        mypath = path(true);
        BOOST_AFIO_ERRHNTFN(NtQueryInformationFile(myid, &isb, fni, sizeof(buffer), FileNameInformation), [this]
                            {
                              return path();
                            });
...
#  endif  /*_ 0 _*/
      ...
      BOOST_AFIO_ERRHNTFN(NtSetInformationFile(myid, &isb, &fdi, sizeof(fdi), FileDispositionInformation), [this]
                          {
                            return path();
                          });
# endif  /*_ 0 _*/
    ...
    BOOST_AFIO_HEADERS_ONLY_VIRTUAL_SPEC void atomic_relink(const async_path_op_req &_req) override final
    {
      ...
      fni->FileNameLength = (ULONG)(req.path.native().size() * sizeof(path::value_type));
      memcpy(fni->FileName, req.path.c_str(), fni->FileNameLength);
      BOOST_AFIO_ERRHNTFN(NtSetInformationFile(myid, &isb, fni, sizeof(FILE_RENAME_INFORMATION) + fni->FileNameLength, FileRenameInformation), [this]
                          {
                            return path();
                          });
    }
  };
  inline async_io_handle_windows::async_io_handle_windows(async_file_io_dispatcher_base *_parent, const BOOST_AFIO_V1_NAMESPACE::path &path, file_flags flags, bool _SyncOnClose, HANDLE _h)
      : async_io_handle(_parent, flags)
  ...
  class async_file_io_dispatcher_windows : public async_file_io_dispatcher_base
  {
    ...
    size_t pagesize;
    std::shared_ptr<async_io_handle> decode_relative_path(async_path_op_req &req, bool force_absolute = false) { return detail::decode_relative_path<async_file_io_dispatcher_windows, async_io_handle_windows>(req, force_absolute); }
...
# else  /*_ 0 _*/
      ...
      BOOST_AFIO_ERRHNTFN(ntstat, [&req]
                          {
                            return req.path;
                          });
      auto untemph = detail::Undoer([&temph]
                                    {
                                      if(temph)
                                        CloseHandle(temph);
                                    });
      ...
      FILE_DISPOSITION_INFORMATION fdi = {true};
      BOOST_AFIO_ERRHNTFN(NtSetInformationFile(temph, &isb, &fdi, sizeof(fdi), FileDispositionInformation), [this]
                          {
                            return path();
                          });
# endif  /*_ 0 _*/
  ...
  private:
    ...
    completion_returntype dofile(size_t id, async_io_op op, async_path_op_req req)
    {
      ...
      BOOST_AFIO_ERRHNTFN(status, [&req]
                          {
                            return req.path;
                          });
      // If writing and SyncOnClose and NOT synchronous, turn on SyncOnClose
      auto ret = std::make_shared<async_io_handle_windows>(this, req.path, req.flags, (file_flags::SyncOnClose | file_flags::Write) == (req.flags & (file_flags::SyncOnClose | file_flags::Write | file_flags::AlwaysSync)), h);
      ...
      if(!!(req.flags & file_flags::CreateCompressed) && (!!(req.flags & file_flags::CreateOnlyIfNotExist) || !!(req.flags & file_flags::Create)))
      {
        ...
        BOOST_AFIO_ERRHWINFN(DeviceIoControl(ret->native_handle(), FSCTL_SET_COMPRESSION, &val, sizeof(val), nullptr, 0, &bytesout, nullptr), [&req]
                             {
                               return req.path;
                             });
      }
...
# endif  /*_ defined(__MINGW32__) && !defined(__MINGW64__) && !defined(__MINGW64_VERSION_MAJOR) _*/
          DWORD bytesout = 0;
    ...
    // Called in unknown thread
    completion_returntype dormfile(size_t id, async_io_op op, async_path_op_req req) { return dounlink(true, id, std::move(op), std::move(req)); }
    // Called in unknown thread
    void boost_asio_symlink_completion_handler(size_t id, std::shared_ptr<async_io_handle> h, std::shared_ptr<std::unique_ptr<path::value_type[]>> buffer, const asio::error_code &ec, size_t bytes_transferred)
    {
      if(ec)
      {
        ...
        try
        {
          BOOST_AFIO_ERRGWINFN(ec.value(), [h]
                               {
                                 return h->path();
                               });
        }
...
# endif  /*_ defined(__MINGW32__) && !defined(__MINGW64__) && !defined(__MINGW64_VERSION_MAJOR) _*/
    ...
    completion_returntype dosync(size_t id, async_io_op op, async_io_op)
    {
      ...
      if(bytestobesynced)
        BOOST_AFIO_ERRHWINFN(FlushFileBuffers(p->native_handle()), [p]
                             {
                               return p->path();
                             });
...
# endif  /*_ defined(DEBUG_PRINTING) _*/
      if(p->mapaddr)
      {
...
# if 1
      BOOST_AFIO_ERRHWINFN(wintruncate(p->native_handle(), _newsize), [p]
                           {
                             return p->path();
                           });
# else  /*_ 1 _*/
      ...
      {
        BOOST_AFIO_ERRHWINFN(SetFilePointerEx(p->native_handle(), newsize, NULL, FILE_BEGIN), [p]
                             {
                               return p->path();
                             });
        BOOST_AFIO_ERRHWINFN(SetEndOfFile(p->native_handle()), [p]
                             {
                               return p->path();
                             });
        BOOST_AFIO_ERRHWINFN(GetFileSizeEx(p->native_handle(), &size), [p]
                             {
                               return p->path();
                             });
      }
      ...
      else
      {
        ...
        if(needmoremetadata)
        {
          for(auto &i : _ret)
          {
            ...
            {
            }  // File may have vanished between enumerate and now
          }
        }
...
# endif  /*_ defined(__MINGW32__) && !defined(__MINGW64__) && !defined(__MINGW64_VERSION_MAJOR) _*/
  ...
  public:
    async_file_io_dispatcher_windows(std::shared_ptr<thread_source> threadpool, file_flags flagsforce, file_flags flagsmask)
        ...
        , pagesize(utils::page_sizes().front())
    {
    }
...
# endif  /*_ BOOST_AFIO_VALIDATE_INPUTS _*/
...
# endif  /*_ BOOST_AFIO_VALIDATE_INPUTS _*/
  ...
  struct win_actual_lock_file : public actual_lock_file
  {
    ...
    win_actual_lock_file(BOOST_AFIO_V1_NAMESPACE::path p)
